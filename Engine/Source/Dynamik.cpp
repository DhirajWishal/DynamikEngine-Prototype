...
namespace Dynamik {
	...
	void DMKEngine::initializeInstance(DMKInstanceDescriptor descriptor)
	{
		...
		instance.myStartypRendererThread.swap(std::thread(
			[](CCPTR iconPath) 
			{
				Dynamik::Renderer::StartupRenderer::loadTexure("E:/Projects/Dynamik Engine/Versions/Dynamik/Dependencies/Assets/icons/Samples/Dynamik.png");
				Dynamik::Renderer::StartupRenderer::initialize();
				Dynamik::Renderer::StartupRenderer::initializeShaders();
				Dynamik::Renderer::StartupRenderer::initializeVertexBuffers();
				Dynamik::Renderer::StartupRenderer::initializeTextureImage();

				while (!finishStartUpRenderer) 
				{ 
					DMK_BEGIN_PROFILE_TIMER();

					Dynamik::Renderer::StartupRenderer::draw();
				}

				Dynamik::Renderer::StartupRenderer::terminate();
	...
	{
		Dynamik::Renderer::Renderer::terminate();
	}
	...
	void DMKEngine::initializeRendererStageOne()
	{
		...
		windowInitInfo.iconPaths = "E:/Projects/Dynamik Engine/Versions/Dynamik/Dependencies/Assets/icons/Samples/Dynamik.png";
		instance.myWindowManager.initialize(windowInitInfo);

		/* Set basic initializing data to the rendering engine */
		Dynamik::Renderer::Renderer::setProgressPointer(&instance.progress);
		Dynamik::Renderer::Renderer::setWindowHandle(instance.myWindowManager.window);
		Dynamik::Renderer::Renderer::setWindowExtent(instance.myWindowManager.windowWidth, instance.myWindowManager.windowHeight);

		/* Initialize the Renderer: Stage One */
		Dynamik::Renderer::Renderer::initializeStageOne(instance.myInstanceDescriptor.renderingAPI, instance.myInstanceDescriptor.settings);
	}
	...
	void DMKEngine::genarateRenderables()
	{
		...
		Dynamik::Renderer::Renderer::setRenderableObjects(instance.internalFormats);
	}

	void DMKEngine::submitLoadedAssets()
	{
		Dynamik::Renderer::Renderer::submitLoadedAssets();
	}
	...
	void DMKEngine::addToRenderingQueue(DMKGameObject* object)
	{
		...
		Dynamik::Renderer::Renderer::addToRenderQueue((InternalFormat*)object);
	}
	...
	void DMKEngine::initializeRendererStageTwo()
	{
		...
		Dynamik::Renderer::Renderer::initializeStageTwo();
	}
	...
	void DMKEngine::initializeRendererStageThree()
	{
		...
		Dynamik::Renderer::Renderer::initializeStageThree();
	}
	...
	void DMKEngine::run()
	{
		...
		while (EventManager::pollEventsGLFW())
		{
			...
			Dynamik::Renderer::Renderer::drawFrame(info);
		}

		/* Clean the rendering engine right after the window gets terminated */
		Dynamik::Renderer::Renderer::frameCleanup();
	}
