...
namespace Dynamik {
	namespace Renderer {
		namespace Backend {
			...
			void VulkanComputeCore::initializePhysicalDevice()
			{
				...
				for (const auto& device : devices) {
					...
					if (isCandidate(physicalDeviceLimits, props))
					{
						...
						physicalDeviceLimits = VulkanComputeShaderPhysicalDeviceLimits(props);
			...
			{
				queueFamilyIndices = VulkanComputeQueue::getQueues(physicalDevice);
			...
			VulkanComputeQueue VulkanComputeQueue::getQueues(VkPhysicalDevice physicalDevice)
			{
				VulkanComputeQueue _queue;
			...
			B1 VulkanComputeQueue::isComplete()
			{
