...
namespace Dynamik {
	...
	void DMKVertex::addAttribute(const DMKVertexAttribute& attribute)
	{
		attributes.pushBack(attribute);
	}
	
	void DMKVertex::updateAttribute(const DMKVertexAttribute attribute, UI32 attributeIndex)
	{
		attributes[attributeIndex] = attribute;
	}

	ARRAY<DMKVertexAttribute> DMKVertex::getAttributes()
	{
		return attributes;
	}
}