#include "adgrafx.h"
#include "VulkanDebuggerManager.h"
...
namespace Dynamik {
	namespace ADGR {
		namespace Backend {
			...
			VkResult VulkanDebuggerManager::createDebugUtilsMessengerEXT(
			...
			void VulkanDebuggerManager::destroyDebugUtilsMessengerEXT(
				...
				if (func != nullptr)
					func(myInstance, core->debugMessenger, pAllocator);
			}

			void VulkanDebuggerManager::populateDebugMessegerCreateInfo(VkDebugUtilsMessengerCreateInfoEXT& createInfo) {
				createInfo = {};
			...
			void VulkanDebuggerManager::initialize(POINTER<VulkanCoreObject> core)
			{
				myInstance = core->instance;
				...
				if (createDebugUtilsMessengerEXT(&createInfo, nullptr, &core->debugMessenger) != VK_SUCCESS)
					DMK_CORE_FATAL("Failed to set up debug messenger!");
			}

			void VulkanDebuggerManager::terminate(POINTER<VulkanCoreObject> core)
			{
				if (VulkanValidator::enableValidationLayers)
