...
namespace Dynamik {
	namespace ADGR {
		namespace Backend {
			void VulkanInstanceManager::initialize(POINTER<VulkanCoreObject> core, ADGRVulkanInstanceInitInfo info)
			{
				...
				if (VulkanValidator::enableValidationLayers) {
					...
					VulkanDebuggerManager _localDebugger;
					VulkanDebuggerManager::populateDebugMessegerCreateInfo(debugCreateInfo);
			...
			void VulkanInstanceManager::terminate(POINTER<VulkanCoreObject> core)
			{
