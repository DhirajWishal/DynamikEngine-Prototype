...
namespace Dynamik {
	namespace ADGR {
		namespace Backend {
			void VulkanFunctions::createBuffer(ADGRCreateBufferInfo info)
			{
				...
				if (vkCreateBuffer(info.device.logicalDevice, &bufferInfo, nullptr, info.buffer) != VK_SUCCESS)
					DMK_CORE_FATAL("failed to create buffer!");

				VkMemoryRequirements memRequirements;
				vkGetBufferMemoryRequirements(info.device.logicalDevice, *info.buffer, &memRequirements);
				...
				if (vkAllocateMemory(info.device.logicalDevice, &allocInfo, nullptr, info.bufferMemory) != VK_SUCCESS)
					DMK_CORE_FATAL("failed to allocate buffer memory!");

				vkBindBufferMemory(info.device.logicalDevice, *info.buffer, *info.bufferMemory, 0);
			}
			...
			void VulkanFunctions::copyBuffer(VkDevice device, VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size, VkCommandPool commandPool, VkQueue graphicsQueue)
			{
			...
			void VulkanFunctions::createImage(ADGRCreateImageInfo info)
			{
				...
				if (vkCreateImage(info.device.logicalDevice, &imageInfo, nullptr, info.image) != VK_SUCCESS)
					DMK_CORE_FATAL("failed to create image!");

				VkMemoryRequirements memRequirements;
				vkGetImageMemoryRequirements(info.device.logicalDevice, *info.image, &memRequirements);

				VkMemoryAllocateInfo allocInfo = {};
				allocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
				allocInfo.allocationSize = memRequirements.size;
				allocInfo.memoryTypeIndex = findMemoryType(memRequirements.memoryTypeBits,
					VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, info.device.physicalDevice);

				if (vkAllocateMemory(info.device.logicalDevice, &allocInfo, nullptr, info.imageMemory) != VK_SUCCESS)
			...
			VkImageView VulkanFunctions::createImageView(ADGRCreateImageViewInfo info)
			{
				...
				if (vkCreateImageView(info.device.logicalDevice, &viewInfo, nullptr, &imageView) != VK_SUCCESS)
